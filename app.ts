...
let canFly : boolean; // only boolean type value will assign, otherwise error will show
...
let getadd = add(20, 2, false, 'javascript');
cl(getadd);
...
function sub(n1 : number, n2 : number, showRes : boolean){
    if(showRes === true){
        cl(n1-n2);
    }else{
        return n1-n2;
